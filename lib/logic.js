...
const process_post = async function(apiKey, threshold, log, data) {
    ...
    await gpt3(apiKey, text, log).then(function(text_embedding) {
...
const logic = async function(data) {
    // Only process posts
    ...
    if (!have_embedded_for_preset_tags) {
        let promises = preset_tags.map(async function(tag) {
            return await gpt3(apiKey, tag, log).then(function(res) {
                preset_tags_embeddings[tag] = res;
            });
        });

        await Promise.all(promises).then(async function() {
            await process_post(apiKey, threshold, log, data);
        });
